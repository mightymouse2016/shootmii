...
Cannon::Cannon(const float _angleMin, const float _angleMax,
		const float _angle, const float _rotationStep, Wind* _wind,
		Player* _owner, int _playerNumber, Manager* _manager) :
	Rectangle(CANNON_LAYER, CANNON_WIDTH, CANNON_HEIGHT, TANK_HEIGHT / 4, 0,
			_angle, 0, 0, true, true, App::imageBank->get(TXT_CANNON), _owner,
			0, CANNON_WIDTH, CANNON_HEIGHT), wind(_wind), heat(0), strength(0),
			angleMin(_angleMin), angleMax(_angleMax), rotationStep(
					_rotationStep), stillHeld(false), reloadTime(new Timer(
					RELOAD_TIME)), blockedCannon(new Timer(BLOCKING_TIME)),
			furyReloadTime(new Timer(FURY_RELOAD_TIME)), manager(_manager),
			t_laser(0), guidedMissile(NULL) {
	...
	addChild(new Rectangle(CROSSHAIR_LAYER, CROSSHAIR_WIDTH, CROSSHAIR_HEIGHT,
			0, 0, CROSSHAIR_OVERTAKE, angle, 0, true, true, crossHair_image,
			_owner));
	addChild(NULL); //< Un emplacement pour la munition
	for (int i = 0; i < STRENGTH_JAUGE_STATES; i++) {
		addChild(
				new Rectangle(STRENGTH_JAUGE_LAYER,
						STRENGHT_JAUGE_SPRITE_WIDTH,
						STRENGHT_JAUGE_SPRITE_HEIGHT, CANNON_WIDTH / 2, 0, i
								* 100 / STRENGTH_JAUGE_STATES, 0, 0, true,
						true, App::imageBank->get(TXT_STRENGTH_SPRITES), this,
						i, STRENGHT_JAUGE_SPRITE_WIDTH,
						STRENGHT_JAUGE_SPRITE_HEIGHT, true));
	}
}
...
void Cannon::init() {
	...
	if (isGuidingMissile())
		guidedMissile = NULL; //< le Manager s'occupe déjà de la suppression
...
void Cannon::computeHeat() {
	if (getOwner()->isInFuryMode()) {
		...
		if (blockedCannon->timeIsOver())
			heat -= HEAT_COOL_SLOW_STEP;
		else
			blockedCannon->compute();
	} else if (heat > 50)
		heat -= HEAT_COOL_SLOW_STEP; //< Mode lent quand le canon est chaud
	else if (heat > 0)
		heat -= HEAT_COOL_FAST_STEP; //< Mode normal 50 premiers % de la jauge
	if (heat < 0)
		heat = 0;
}
...
void Cannon::incStrength() {
	if (getOwner()->isInFuryMode())
		return;
	if (!isLoaded())
		return; //< Si il n'y a pas de munition dans le canon
	if (heat == 100)
		return;
...
void Cannon::computeStrengthJauge() {
	...
	for (int i = 0; i < showLimit; i++)
		children[CHILDREN_STRENGTH + i]->show();
	for (int i = showLimit; i < STRENGTH_JAUGE_STATES; i++)
...
void Cannon::computeLaserMode() {
	...
	if (!getOwner()->isInLaserMode())
		return;

	// Permet de faire défiler les pointillés
	t_laser++;
	if (t_laser * LASER_MOVE_STEP > 2 * LASER_STEP)
		t_laser = 0;

	float angle = getAbsolutePolygonAngle(), cosinus = cos(angle), sinus = sin(
			angle);
	laserXMax = new PolyDeg2(wind->getWindSpeed() * WIND_INFLUENCE_ON_AMMO / (2
			* 100 * AMMO_WEIGHT), 100 * cosinus, getAbsoluteOriginX()
			+ CANNON_LENGTH * cosinus);
	laserYMax = new PolyDeg2(-GRAVITY / (2 * AMMO_WEIGHT), 100 * sinus,
			getAbsoluteOriginY() + CANNON_LENGTH * sinus);

	if (strength == 0)
		return;

	laserX = new PolyDeg2(wind->getWindSpeed() * WIND_INFLUENCE_ON_AMMO / (2
			* 100 * AMMO_WEIGHT), strength * cosinus, getAbsoluteOriginX()
			+ CANNON_LENGTH * cosinus);
	laserY = new PolyDeg2(-GRAVITY / (2 * AMMO_WEIGHT), strength * sinus,
			getAbsoluteOriginY() + CANNON_LENGTH * sinus);
}
...
void Cannon::shoot() {
	...
	if (getOwner()->isInFuryMode())
		...
		return; //< Si le canon est trop chaud, on ne peux pas tirer
	getAmmo()->init(strength);
	guidedMissile = dynamic_cast<GuidedMissile*> (getAmmo()); //< Cas ou l'on a un missile télé-guidé
	setAmmo(NULL);
	if (strength < SHOT_MINIMUM_STRENGTH)
		App::soundBank->play(AMMO_SHOT, SHOT_MINIMUM_SOUND);
	else
		App::soundBank->play(AMMO_SHOT, strength);
	strength = 0;
...
void Cannon::computeReload() {
	...
	if (getOwner()->isInFuryMode()) {
		...
		if (furyReloadTime->timeIsOver()) {
			if (rand() % FURY_HOMING_PROBABILITY)
				loadCannon();
			else
				loadHoming();
		}
		...
		reloadTime->compute();
		if (reloadTime->timeIsOver() && !stillHeld)
			loadCannon();
	}
}
...
void Cannon::loadCannon() {
	...
	case 1:
		ammoImage = App::imageBank->get(TXT_AMMO1);
		break;
	case 2:
		ammoImage = App::imageBank->get(TXT_AMMO2);
		break;
	default:
		ammoImage = App::imageBank->get(TXT_AMMO1);
		break;
...
void Cannon::loadHoming() {
	...
	if (isLoaded())
		delete children[CHILD_AMMO];
	Player* owner = getOwner();
	GRRLIB_texImg* ammoImage = NULL;
	switch (owner->getPlayerNumber()) {
	case 1:
		ammoImage = App::imageBank->get(TXT_HOMING1);
		break;
	case 2:
		ammoImage = App::imageBank->get(TXT_HOMING2);
		break;
	default:
		ammoImage = App::imageBank->get(TXT_HOMING1);
		break;
	}
	setAmmo(new HomingMissile(angle, wind, ammoImage, owner,
			owner->getTerrain(), manager));
}
...
void Cannon::loadGuided() {
	...
	switch (owner->getPlayerNumber()) {
	case 1:
		ammoImage = App::imageBank->get(TXT_GUIDED1);
		break;
	case 2:
		ammoImage = App::imageBank->get(TXT_GUIDED2);
		break;
	default:
		ammoImage = App::imageBank->get(TXT_GUIDED1);
		break;
	}
	setAmmo(new GuidedMissile(angle, wind, ammoImage, owner,
			owner->getTerrain(), manager));
}
...
void Cannon::draw() const{
	...
	if (laserXMax != NULL && laserYMax != NULL) {
		...
		for (int t = -2; newY < terrain->getHeight(newX); t++) {
			if (t % 2)
				GRRLIB_Line(x, y, newX, newY, Color::BLACK);
			x = newX;
			y = newY;
	...
	if (strength != 0 && laserX != NULL && laserY != NULL) {
		...
		for (int t = 0; newY < terrain->getHeight(newX); t++) {
			GRRLIB_Line(x, y, newX, newY, Color::WHITE);
			x = newX;
			y = newY;
