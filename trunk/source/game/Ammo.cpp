...
Ammo::Ammo(const float _angle, Function* _calcX, Function* _calcY) :
	calcX(_calcX), calcY(_calcY), t(0), angle(_angle), destroyed(false) {
}
...
CannonBall::CannonBall(const float _angle, Wind* wind, const float _vInitX, const float _vInitY,
		const float _xInit, const float _yInit) :
	Ammo(_angle, new PolyDeg2(float(wind->getWindSpeed()) * (wind->getWindDirection()
			* 2 - 1) * WIND_INFLUENCE_ON_AMMO / (2* 100* AMMO_WEIGHT ),_vInitX,_xInit) ,new PolyDeg2(-G/(2*AMMO_WEIGHT),-_vInitY,_yInit)){
	}
